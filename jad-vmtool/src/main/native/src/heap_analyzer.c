 ...
 * - JAD 的 vmtool 其它功能也会使用 JVMTI tag；因此这里采用“带 magic + run_id 的数值 tag”，并在结束后恢复原 tag，避免冲突
...
char *jad_vmtool_heap_analyze(jvmtiEnv *jvmti, jint class_num,
                                 jint object_num) {
  heap_ctx_t ctx;
  jvmtiHeapCallbacks callbacks;
  jvmtiError err;
...
char *jad_vmtool_reference_analyze(jvmtiEnv *jvmti, jclass klass,
