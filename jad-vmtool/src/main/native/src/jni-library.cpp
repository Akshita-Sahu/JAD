...
#include <jni_md.h>
#include <jvmti.h>
#include "jad_VmTool.h" // under target/native/javah/
...
int init_agent(JavaVM *vm, void *reserved) {
    ...
    if (rc != JNI_OK) {
        fprintf(stderr, "ERROR: jad vmtool Unable to create jvmtiEnv, GetEnv failed, error=%d\n", rc);
        return -1;
    }
    ...
    if (error) {
        fprintf(stderr, "ERROR: jad vmtool JVMTI AddCapabilities failed!%u\n", error);
        return JNI_FALSE;
    }
...
Java_jad_VmTool_forceGc0(JNIEnv *env, jclass thisClass) {
...
Java_jad_VmTool_getInstances0(JNIEnv *env, jclass thisClass, jclass klass, jint limit) {
    jlong tag = getTag();
...
Java_jad_VmTool_sumInstanceSize0(JNIEnv *env, jclass thisClass, jclass klass) {
    jlong tag = getTag();
    limitCounter.init(-1);
    jvmtiError error = jvmti->IterateOverInstancesOfClass(klass, JVMTI_HEAP_OBJECT_EITHER,
                                               HeapObjectCallback, &tag);
...
extern "C"
JNIEXPORT jlong JNICALL Java_jad_VmTool_getInstanceSize0
...
extern "C"
JNIEXPORT jlong JNICALL
Java_jad_VmTool_countInstances0(JNIEnv *env, jclass thisClass, jclass klass) {
    jlong tag = getTag();
    limitCounter.init(-1);
    jvmtiError error = jvmti->IterateOverInstancesOfClass(klass, JVMTI_HEAP_OBJECT_EITHER,
                                               HeapObjectCallback, &tag);
...
extern "C"
JNIEXPORT jobjectArray JNICALL Java_jad_VmTool_getAllLoadedClasses0
...
extern "C"
JNIEXPORT jint JNICALL Java_jad_VmTool_mallocTrim0
...
extern "C"
JNIEXPORT jboolean JNICALL Java_jad_VmTool_mallocStats0
        (JNIEnv *env, jclass thisClass) {
#ifdef __GLIBC__
    ::malloc_stats();
    return JNI_TRUE;
...
extern "C"
JNIEXPORT jstring JNICALL Java_jad_VmTool_heapAnalyze0
        (JNIEnv *env, jclass thisClass, jint classNum, jint objectNum) {
    (void)thisClass;
    char *result = jad_vmtool_heap_analyze(jvmti, classNum, objectNum);
    if (!result) {
...
extern "C"
JNIEXPORT jstring JNICALL Java_jad_VmTool_referenceAnalyze0
        (JNIEnv *env, jclass thisClass, jclass klass, jint objectNum, jint backtraceNum) {
    (void)thisClass;
    char *result = jad_vmtool_reference_analyze(jvmti, klass, objectNum, backtraceNum);
    if (!result) {
