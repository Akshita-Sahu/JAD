...
REM don't call 'echo' before 'start jad.bat'
...
REM Runas Service, don't call 'echo' before 'start jad.bat'
...
if %as_service%==1 (
	REM run jad.bat
	start /wait %basedir%\jad.bat %pid% %as_args%
	exit 0
	
	REM DEBUG run args
	REM echo as_args: %as_args%
	REM echo start /wait %basedir%\jad.bat %pid% %as_args%
	REM exit /b 0
)
...
IF %ERRORLEVEL% EQU 0 (
	echo JAD agent already running, just connect to it!
	goto :attachSuccess
)
...
echo Preparing jad service and injecting jad agent to process: %pid% ...
...
echo jad srv type: %srv_type%
echo jad srv binPath: %srv_binpath%

sc create %srv_name% start= demand %srv_type% %srv_binpath%
sc config %srv_name% start= demand %srv_type% %srv_binpath%
if %errorlevel% NEQ 0 (
	echo Config JAD service failed
	exit /b -1
)

sc stop %srv_name%
REM fork start JAD service, avoid blocking
if %srv_interact%==1 (
...
REM check and connect jad ..
echo Waitting for jad agent ...
set count=0
...
IF %ERRORLEVEL% NEQ 0 (
    set /a count+=1
    if %count% geq 8 (
        echo JAD agent telnet port is not ready, maybe inject failed.
        goto :end
    )
    ping -w 1 -n 2 0.0.0.0 > nul
    goto :waitfor_loop
)
echo JAD agent telnet port is ready.
...
IF %ERRORLEVEL% NEQ 0 (
  ...
  ECHO Try to visit http://127.0.0.1:%http_port% to connecto jad server.
...
echo(
echo Checking jad telnet port [:%telnet_port%] ...
netstat -nao |findstr LIST |findstr :%telnet_port%
IF %ERRORLEVEL% EQU 0 (
	echo JAD agent is still running!
	goto :choice
) else (
	echo JAD agent is shutdown.
	goto :end
)

:choice
set /P c=Are you going to shutdown jad agent [Y/N]?
echo input: %c%
...
:shutdown_agent
echo Shutting down jad ...
%JAVACMD% -jar jad-client.jar -c shutdown 127.0.0.1 %telnet_port%
...
netstat -nao |findstr LIST |findstr :%telnet_port%
IF %ERRORLEVEL% EQU 0 (
	echo JAD shutdown failed! 
) else (
	echo JAD shutdown successfully.
)
goto :end
...
echo   -telnet-port port  : Change jad telnet port 
echo   -http-port port    : Change jad http/websocket port 
echo   --interact         : Enable windows service interactive UI, useful for debug
echo   --remove           : Remove JAD windows service
...
echo   %filename% --remove  #remove jad service
exit /b -1
