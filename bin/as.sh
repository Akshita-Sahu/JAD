...
# program : JAD
#  author : Core Engine @ Akshita Sahu.com
#    date : 2026-03-09

# current jad script version
JAD_SCRIPT_VERSION=4.1.8
...
# define jad's lib
if [ -z "${JAD_LIB_DIR}" ]; then
    JAD_LIB_DIR=${HOME}/.jad/lib
else
    echo "[INFO] JAD_LIB_DIR: ${JAD_LIB_DIR}"
fi
...
# remote repo to download jad
REPO_MIRROR=

# use http to download jad
USE_HTTP=false
...
# jad-client terminal height
HEIGHT=
# jad-client terminal width
WIDTH=
...
# define jad's temp dir
TMP_DIR=/tmp

# jad remote url
# https://github.com/Akshita-Sahu/JAD/download/3.1.7?mirror=akshita-sahu
REMOTE_DOWNLOAD_URL="https://github.com/Akshita-Sahu/JAD/download/PLACEHOLDER_VERSION?mirror=PLACEHOLDER_REPO"
# update timeout(sec)
SO_TIMEOUT=5
...
JAD_OPTS="-Djava.awt.headless=true"
...
if ! [ -x "$(command -v curl)" ]; then
  echo 'Error: curl is not installed. Try to use java -jar jad-boot.jar' >&2
  exit 1
fi
if ! [ -x "$(command -v grep)" ]; then
  echo 'Error: grep is not installed. Try to use java -jar jad-boot.jar' >&2
  exit 1
fi
if ! [ -x "$(command -v awk)" ]; then
  echo 'Error: awk is not installed. Try to use java -jar jad-boot.jar' >&2
  exit 1
fi
if ! [ -x "$(command -v telnet)" ]; then
  echo 'Error: telnet is not installed. Try to use java -jar jad-boot.jar' >&2
  exit 1
fi
if ! [ -x "$(command -v unzip)" ]; then
  echo 'Error: unzip is not installed. Try to use java -jar jad-boot.jar' >&2
  exit 1
fi
...
reset_for_env()
{
    ...
    # init JAD' lib
    mkdir -p "${JAD_LIB_DIR}" \
        || exit_on_err 1 "create ${JAD_LIB_DIR} fail."

    # if env define the JAVA_HOME, use it first
    # if is akshita-sahu opts, use akshita-sahu ops's default JAVA_HOME
    [ -z "${JAVA_HOME}" ] && [ -d /opt/akshita-sahu/java ] && JAVA_HOME=/opt/akshita-sahu/java
    ...
    # iterater through candidates to find a proper JAVA_HOME at least contains tools.jar which is required by jad.
    if [ ! -d "${JAVA_HOME}" ]; then
    ...
    if [[ "$JAVA_VERSION" -lt 9 ]];then
      ...
      [ -z "${BOOT_CLASSPATH}" ] && exit_on_err 1 "tools.jar was not found, so jad could not be launched!"
    fi

    echo "[INFO] JAVA_HOME: ${JAVA_HOME}"

    # reset CHARSET for akshita-sahu opts, we use GBK
    [[ -x /opt/akshita-sahu/java ]] && JVM_OPTS="-Dinput.encoding=GBK ${JVM_OPTS} "
...
get_remote_version()
{
    curl -sLk "https://github.com/Akshita-Sahu/JAD/api/latest_version"
}
...
version_gt()
{
    ...
    local jad_local_version=$2
    [[ "$remote_version" > "$jad_local_version" ]] && return 0 || return 1
}

# update jad if necessary
update_if_necessary()
{
    local update_version=$1

    if [ ! -d "${JAD_LIB_DIR}/${update_version}" ]; then
        echo "updating version ${update_version} ..."

        local temp_target_lib_dir="$TMP_DIR/temp_${update_version}_$$"
        local temp_target_lib_zip="${temp_target_lib_dir}/jad-${update_version}-bin.zip"
        local target_lib_dir="${JAD_LIB_DIR}/${update_version}/jad"
        ...
        echo "Download jad from: ${downloadUrl}"
        curl \
            -#Lk \
        ...
        if ! (unzip "${temp_target_lib_zip}" -d "${temp_target_lib_dir}") ; then
            rm -rf "${temp_target_lib_dir}" "${JAD_LIB_DIR}/${update_version}"
            return 1
        fi

        mkdir -p "${JAD_LIB_DIR}/${update_version}"
        # rename
...
Usage:
    $0 [-h] [--target-ip <value>] [--telnet-port <value>]
       [--http-port <value>] [--session-timeout <value>] [--jad-home <value>]
...
NOTE: JAD 4 supports JDK 8+. If you need to diagnose applications running on JDK 6/7, you can use JAD 3.
...
Options and Arguments:
 -h,--help                      Print usage
    ...
    --jad-home <value>       The jad home
    --use-version <value>       Use special version jad
    --repo-mirror <value>       Use special remote repository mirror, value is
                                center/akshita-sahu or http repo url.
    --versions                  List local and remote jad versions
 ...
 -f,--batch-file <value>        The batch file to execute
    --height <value>            jad-client terminal height
    --width <value>             jad-client terminal width
 -v,--verbose                   Verbose, print debug info.
 <pid>                          Target pid

EXAMPLES:
  ./jad.sh <pid>
  ./jad.sh --telnet-port 9999 --http-port -1
  ./jad.sh --username admin --password <password>
  ./jad.sh --tunnel-server 'ws://192.168.10.11:7777/ws' --app-name demoapp
  ./jad.sh --tunnel-server 'ws://192.168.10.11:7777/ws' --agent-id bvDOe8XbTM2pQWjF4cfw
  ./jad.sh --stat-url 'http://192.168.10.11:8080/api/stat'
  ./jad.sh -c 'sysprop; thread' <pid>
  ./jad.sh -f batch.as <pid>
  ./jad.sh --use-version 4.1.8
  ./jad.sh --session-timeout 3600
  ./jad.sh --attach-only
  ./jad.sh --disabled-commands stop,dump
  ./jad.sh --select math-game
  ./jad.sh --repo-mirror akshita-sahu --use-http
WIKI:
...
list_versions()
{
    echo "JAD versions under ${JAD_LIB_DIR}:"
    ls -1 "${JAD_LIB_DIR}"
}
...
# Status from com.akshita.jad.client.TelnetConsole
...
# maybe another user start an jad server at the same port, but invisible for current user
find_listen_port_process_by_client()
{
    local jad_lib_dir="${JAD_HOME}"
    # http://www.inonit.com/cygwin/faq/
    if [ "${OS_TYPE}" = "Cygwin" ]; then
        jad_lib_dir=`cygpath -wp "$jad_lib_dir"`
    fi

    "${JAVA_HOME}/bin/java" ${JAD_OPTS} ${JVM_OPTS} \
             -jar "${jad_lib_dir}/jad-client.jar" \
             $(getTargetIPOrDefault) \
             $(getTelnetPortOrDefault) \
             -c "session" \
...
parse_arguments()
{
    ...
    if [ -z ${REPO_MIRROR} ]; then
        REPO_MIRROR="center"
        # if timezone is +0800, set REPO_MIRROR to akshita-sahu
        if [[ -x "$(command -v date)" ]] && [[  $(date +%z) == "+0800" ]]; then
            REPO_MIRROR="akshita-sahu"
        fi
    fi
    ...
    if [ -z ${TARGET_PID} ]; then
        ...
        for process in "${CANDIDATES[@]}"; do
            ...
            if [ $(echo ${process} | grep -c org.apache.catalina.startup.Bootstrap) -eq 1 ] \
                || [ $(echo ${process} | grep -c com.akshita-sahu.pandora.boot.loader.SarLauncher) -eq 1 ]
            then
               suggest=${index}
               break
            fi
        ...
        if [[ ($httpPortPid) && ($TARGET_PID != $httpPortPid) ]]; then
            ...
            echo "1. Try to restart jad.sh, select process $httpPortPid, shutdown it first with running the 'stop' command."
            echo "2. Try to use different http port, for example: jad.sh --telnet-port 9998 --http-port 9999"
            exit 1
        fi
...
attach_jvm()
{
    ...
    "${java_command[@]}" \
        ${JAD_OPTS} ${JVM_OPTS} \
        -jar "${jad_lib_dir}/jad-core.jar" \
            -pid ${TARGET_PID} \
            "${tempArgs[@]}" \
            -core "${jad_lib_dir}/jad-core.jar" \
            -agent "${jad_lib_dir}/jad-agent.jar"
...
sanity_check() {
    ...
    if [ "$current_user" != "$target_user" ]; then
        ...
        echo "  1) sudo su $target_user && ./jad.sh"
        echo "  2) sudo -u $target_user -EH ./jad.sh"
        exit_on_err 1
    fi
}
...
print_telnet_port_pid_error() {
    ...
    echo "[ERROR] 1. Try to restart jad.sh, select process $telnetPortPid, shutdown it first with running the 'stop' command."
    echo "[ERROR] 2. Try to stop the existing jad instance: java -jar jad-client.jar 127.0.0.1 $(getTelnetPortOrDefault) -c \"stop\""
    echo "[ERROR] 3. Try to use different telnet port, for example: jad.sh --telnet-port 9998 --http-port -1"
}
...
print_telnet_port_used_error() {
    ...
    echo "[ERROR] Try to use different telnet port, for example: jad.sh --telnet-port 9998 --http-port -1"
}
...
active_console()
{
    ...
    if [ "${BATCH_MODE}" = "true" ]; then
        ...
        echo "telnet connecting to jad server... current timestamp is `date +%s`"
...
main()
{
    echo "JAD script version: $JAD_SCRIPT_VERSION"
    ...
    # try to find jad home from --use-version
    if [[ (-z "${JAD_HOME}")  && (! -z "${USE_VERSION}") ]]; then
        if [[ ! -d "${JAD_LIB_DIR}/${USE_VERSION}/jad" ]] ; then
            update_if_necessary "${USE_VERSION}" || echo "update fail, ignore this update." 1>&2
        fi
        JAD_HOME="${JAD_LIB_DIR}/${USE_VERSION}/jad"
    fi

    # try to set jad home from jad.sh directory
    if [ -z "${JAD_HOME}" ] ; then
        [[ -a "${DIR}/jad-core.jar" ]] \
        && [[ -a "${DIR}/jad-agent.jar" ]] \
        && [[ -a "${DIR}/jad-spy.jar" ]] \
    ...
    # try to find jad under ~/.jad/lib
    if [ -z "${JAD_HOME}" ] ; then
        local remote_version=$(get_remote_version)
        local jad_local_version=$(get_local_version)
        if $(version_gt $remote_version $jad_local_version) ; then
            update_if_necessary "${remote_version}" || echo "update fail, ignore this update." 1>&2
        fi
        local jad_local_version=$(get_local_version)
        JAD_HOME="${JAD_LIB_DIR}/${jad_local_version}/jad"
    fi

    echo "JAD home: ${JAD_HOME}"

    if [ ! -d "${JAD_HOME}" ] ; then
        exit_on_err 1 "JAD home is not a directory, please delete it and retry."
    fi
    ...
    time (attach_jvm "${JAD_HOME}" || exit 1)

    if [ $? -ne 0 ]; then
        exit_on_err 1 "attach to target jvm (${TARGET_PID}) failed, check ${HOME}/logs/jad/jad.log or stderr of target jvm for any exceptions."
    fi

    echo "Attach success."

    if [ ${ATTACH_ONLY} = false ]; then
      active_console "${JAD_HOME}"
    fi
}
