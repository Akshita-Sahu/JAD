...
REM ----------------------------------------------------------------------------
REM  program : JAD
REM   author : Core Engine @ Akshita Sahu.com
REM     date : 2015-11-11
REM  version : 3.0
REM ----------------------------------------------------------------------------
...
set AGENT_JAR=%BASEDIR%\jad-agent.jar
set CORE_JAR=%BASEDIR%\jad-core.jar
...
if %ignoreTools% == 1 (
  ...
  if not exist "%JAVA_HOME%\lib\tools.jar" (
    ...
    echo If java version ^>^= 9, try to run jad.bat ^<pid^> --ignore-tools
    goto exit_bat
  )
