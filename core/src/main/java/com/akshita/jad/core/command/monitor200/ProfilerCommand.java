...
import com.akshita.jad.common.OSUtils;
import com.akshita.jad.core.command.Constants;
import com.akshita.jad.core.command.model.ProfilerModel;
...
/**
 * https://github.com/async-profiler/async-profiler/blob/master/docs/ProfilerOptions.md ， async-profiler agent
...
public class ProfilerCommand extends AnnotatedCommand {
    ...
    private AsyncProfiler profilerInstance() {
        ...
        if (libPath != null) {
            // load from jad directory
            // lib，attach Native Library already loaded in another classloader
            ...
            if (OSUtils.isLinux() || OSUtils.isMac()) {
                throw new IllegalStateException("Can not find libasyncProfiler so, please check the jad directory.");
            } else {
        ...
        try {
            ...
            if (ProfilerAction.execute.equals(profilerAction)) {
                ...
                if (this.duration == null) {
                    ...
                    // stop
                    JADBootstrap.getInstance().getScheduledExecutorService().schedule(new Runnable() {
                        @Override
                        public void run() {
                            // ，profiler，
                            try {
    ...
    private ProfilerModel processStop(AsyncProfiler asyncProfiler, ProfilerAction profilerAction) throws IOException {
        // profiler stop --file xxx.md： Markdown 
    ...
    private ProfilerModel processStopMarkdown(AsyncProfiler asyncProfiler, ProfilerAction profilerAction) throws IOException {
        // Markdown ： async-profiler  collapsed ， JAD 。
        String userFormat = this.format;
        String userFile = this.file;
        int topN = mdTopN(userFormat);

        // stop  start  file，， stop 
        if (profilerAction == ProfilerAction.stop) {
            fileSpecifiedAtStart = null;
        }

        File collapsedFile = File.createTempFile("jad-profiler-collapsed", ".txt");
        String collapsed;
        try {
            //  async-profiler  file  OK  collapsed ，。
    ...
    private String outputFile() throws IOException {
        if (this.file == null) {
            ...
            File outputPath = JADBootstrap.getInstance().getOutputPath();
            if (outputPath != null) {
                this.file = new File(outputPath,
                        new SimpleDateFormat("yyyyMMdd-HHmmss").format(new Date()) + "." + fileExt)
                                .getAbsolutePath();
            } else {
                this.file = File.createTempFile("jad-output", "." + fileExt).getAbsolutePath();
            }
        }
        return file;
    }
