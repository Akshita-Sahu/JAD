...
public class Helper {
    ...
    public static InputStream loadInputRcFile() {
        ...
        try {
            String customInputrc = System.getProperty("user.home") + "/.jad/conf/inputrc";
        ...
        // step 2: load jad default keymap file
        inputrc = TermServer.class.getClassLoader().getResourceAsStream(ShellServerOptions.DEFAULT_INPUTRC);
        if (inputrc != null) {
            logger.info("Loaded jad keymap file from " + ShellServerOptions.DEFAULT_INPUTRC);
            return inputrc;
        }
