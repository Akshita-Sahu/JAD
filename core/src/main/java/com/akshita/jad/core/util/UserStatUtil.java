...
public class UserStatUtil {
    ...
    private static final ExecutorService executorService = Executors.newSingleThreadExecutor(new ThreadFactory() {
        ...
        public Thread newThread(Runnable r) {
            final Thread t = new Thread(r, "jad-UserStat");
            t.setDaemon(true);
            return t;
        }
    });
    ...
    private static final String version = URLEncoder.encode(JADBanner.version().replace("\n", ""));
    ...
    private static void jadUsage(String cmd, String detail, String userId) {
        RemoteJob job = new RemoteJob();
        job.appendQueryData("ip", ip);
        job.appendQueryData("version", version);
        if (agentId != null) {
     ...
     */
    public static void jadUsageSuccess(String cmd, List<String> args, String userId) {
        if (statUrl == null) {
            return;
        }
        ...
        UserStatUtil.jadUsage(cmd, commandString.toString(), userId);
    }

    public static void jadUsageSuccess(String cmd, List<String> args) {
        jadUsageSuccess(cmd, args, null);
    }
