...
import com.akshita.jad.core.distribution.impl.PackingResultDistributorImpl;
import com.akshita.jad.core.distribution.impl.ResultConsumerImpl;
import com.akshita.jad.core.distribution.impl.SharingResultDistributorImpl;
...
import com.akshita.jad.core.shell.system.JobController;
import com.akshita.jad.core.shell.system.JobListener;
import com.akshita.jad.core.shell.system.impl.InternalCommandManager;
...
public class HttpApiHandler {
    ...
    public HttpResponse handle(ChannelHandlerContext ctx, FullHttpRequest request) throws Exception {
        ...
        try {
            ...
            logger.error("jad process http api request error: " + request.uri() + ", request body: " + requestBody, e);
        }
        if (result == null) {
    ...
    private ApiResponse processRequest(ChannelHandlerContext ctx, ApiRequest apiRequest) {
        ...
        try {
            ...
            if (httpSession != null) {
                Object subject = httpSession.getAttribute(JADConstants.SUBJECT_KEY);
                if (subject != null) {
                    session.put(JADConstants.SUBJECT_KEY, subject);
                }
                // get userId from httpSession
                Object userId = httpSession.getAttribute(JADConstants.USER_ID_KEY);
    ...
    private ApiResponse processInitSessionRequest(ApiRequest apiRequest) throws ApiException {
        ...
        if (session != null) {
            ...
            resultDistributor.appendResult(new MessageModel("Welcome to jad!"));

            //welcome message
            WelcomeModel welcomeModel = new WelcomeModel();
            welcomeModel.setVersion(JADBanner.version());
            welcomeModel.setWiki(JADBanner.wiki());
            welcomeModel.setTutorials(JADBanner.tutorials());
