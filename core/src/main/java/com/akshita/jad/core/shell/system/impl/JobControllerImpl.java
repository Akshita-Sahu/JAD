...
import com.akshita.jad.core.shell.command.internal.RedirectHandler;
import com.akshita.jad.core.shell.command.internal.StdoutHandler;
import com.akshita.jad.core.shell.command.internal.TermHandler;
...
import com.akshita.jad.core.shell.system.impl.ProcessImpl.ProcessOutput;
...
public class JobControllerImpl implements JobController {
    ...
    private void checkPermission(Session session, CliToken token) {
        if (JADBootstrap.getInstance().getSecurityAuthenticator().needLogin()) {
            // session Subject
            Object subject = session.get(JADConstants.SUBJECT_KEY);
            if (subject == null) {
                if (token != null && token.isText() && token.value().trim().equals(JADConstants.AUTH)) {
                    // auth 
                    return;
                }
