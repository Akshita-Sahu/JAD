...
public class NettyWebsocketTtyBootstrap {
    ...
    public void start(Consumer<TtyConnection> handler, final Consumer<Throwable> doneHandler) {
        group = new NioEventLoopGroup(new DefaultThreadFactory("jad-NettyWebsocketTtyBootstrap", true));
        ...
        ChannelFuture bindLocalFuture = b2.bind(new LocalAddress(JADConstants.NETTY_LOCAL_ADDRESS));
        if (this.port < 0) { // doneHandler
