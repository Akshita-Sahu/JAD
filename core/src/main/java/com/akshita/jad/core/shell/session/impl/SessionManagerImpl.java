package com.akshita.jad.core.shell.session.impl;
...
public class SessionManagerImpl implements SessionManager {
    ...
    @Override
    public void close() {
        //TODO clear resources while shutdown jad
        closed = true;
        ...
        for (Session session : sessions) {
            SharingResultDistributor resultDistributor = session.getResultDistributor();
            if (resultDistributor != null) {
                resultDistributor.appendResult(new MessageModel("jad server is going to shutdown."));
            }
        ...
        if (!closed && reaperInterval > 0) {
            scheduledExecutorService = Executors.newSingleThreadScheduledExecutor(new ThreadFactory() {
                ...
                public Thread newThread(Runnable r) {
                    final Thread t = new Thread(r, "jad-session-manager");
                    t.setDaemon(true);
                    return t;
                }
            });
