...
JAD_CLASSLOADER_CLASSNAME = "com.akshita.jad.agent.JADClassloader"
JMAP_HISTO_LINE_RE = re.compile(r"^\s*\d+:\s+(?P<instances>\d+)\s+(?P<bytes>\d+)\s+(?P<class>\S+)\s*$")
JAD_LOG_RANDOM_TELNET_PORT_RE = re.compile(r"generate random telnet port: (\d+)")
JAD_LOG_BIND_TELNET_PORT_RE = re.compile(r"try to bind telnet server, host: .* port: (\d+)")


def find_repo_root(start: Path) -> Path:
    for path in [start] + list(start.parents):
        if (path / "pom.xml").is_file() and (path / "bin" / "jad.sh").exists():
            return path
    raise RuntimeError(f"无法定位仓库根目录(未找到 pom.xml/bin/jad.sh)，start={start}")
...
def parse_jad_classloader_instances(jmap_histo_output: str) -> Tuple[int, Optional[str]]:
    matched_line: Optional[str] = None
    instances = 0
    for line in jmap_histo_output.splitlines():
        if JAD_CLASSLOADER_CLASSNAME not in line:
            continue
        m = JMAP_HISTO_LINE_RE.match(line)
        if not m:
            matched_line = line.strip()
            continue
        if m.group("class") != JAD_CLASSLOADER_CLASSNAME:
            continue
        matched_line = line.strip()
...
def wait_for_telnet_port_from_jad_log(jad_log: Path, start_offset: int, timeout_seconds: int) -> int:
    deadline = time.time() + timeout_seconds
    while time.time() < deadline:
        if not jad_log.exists():
            time.sleep(0.2)
            continue

        with open(jad_log, "rb") as f:
            f.seek(start_offset)
            chunk = f.read()
        if not chunk:
            time.sleep(0.2)
        ...
        matches = JAD_LOG_RANDOM_TELNET_PORT_RE.findall(text)
        if matches:
            return int(matches[-1])

        matches = JAD_LOG_BIND_TELNET_PORT_RE.findall(text)
        if matches:
            return int(matches[-1])

        time.sleep(0.2)

    raise TimeoutError(f"未能从 jad.log 解析 telnet 端口: {jad_log}")


def main() -> int:
    parser = argparse.ArgumentParser(description="JAD telnet 循环 stop 后类加载器泄露检查（python + expect + jmap）")
    ...
    parser.add_argument("--attach-timeout", type=int, default=90, help="jad.sh attach 超时秒数(默认: 90)")
    ...
    parser.add_argument("--jad-bin-dir", type=str, default="", help="jad-bin 目录(默认: packaging/target/jad-bin)")
    ...
    if args.work_dir:
        ...
        run_dir = Path(tempfile.mkdtemp(prefix="jad-telnet-stop-leak-"))
    ...
    jad_bin_dir = Path(args.jad_bin_dir) if args.jad_bin_dir else repo_root / "packaging" / "target" / "jad-bin"
    if not jad_bin_dir.is_absolute():
        jad_bin_dir = (repo_root / jad_bin_dir).resolve()
    as_sh = jad_bin_dir / "jad.sh"
    math_game_jar = jad_bin_dir / "math-game.jar"
    if not as_sh.exists():
    ...
    expect_script = repo_root / "integration-test" / "telnet-stop-leak" / "jad_telnet.exp"
    ...
    try:
        ...
        with open(results_csv, "w", newline="", encoding="utf-8") as csv_file:
            ...
            for i in range(1, args.iterations + 1):
                ...
                jad_log = home_dir / "logs" / "jad" / "jad.log"
                start_offset = jad_log.stat().st_size if jad_log.exists() else 0
                ...
                telnet_port = wait_for_telnet_port_from_jad_log(jad_log, start_offset, args.port_wait_timeout)
                try:
                ...
                instances, matched_line = parse_jad_classloader_instances(jmap_histo_out)
                ...
                writer.writerow(
                    {
                        ...
                        "jad_classloader_instances": instances,
                        "matched_histo_line": matched_line or "",
                    }
                )
                csv_file.flush()

                print(f"[INFO] [{i}/{args.iterations}] {JAD_CLASSLOADER_CLASSNAME} instances={instances}")
