...
class JADMcpJavaSdkIT {
    ...
    void setUp() throws Exception {
        ...
        this.env = Environment.start("jad-mcp-java-sdk-it", "jad-mcp-java-sdk-it-home");
    }
    ...
    void should_list_all_mcp_tools_via_java_mcp_sdk() {
        ...
        assertThat(missing).as("tools/list : %s", missing).isEmpty();
        assertThat(extra).as("tools/list : %s", extra).isEmpty();
    }
    ...
    void should_call_stop_tool_via_java_mcp_sdk() throws Exception {
        ...
        try {
            stopEnv = Environment.start("jad-mcp-java-sdk-stop-it", "jad-mcp-java-sdk-stop-it-home");
        ...
        if (isStreamableTool(toolName)) {
            ...
            if (node != null && node.isObject()) {
                ...
                if (resultCount != null && resultCount.canConvertToInt()) {
                    ...
                    Assertions.fail("streamable tool  resultCount: tool=" + toolName + ", body=" + body);
                }
            }
        }
    }
    ...
    private static Map<String, Object> createArgumentsForTool(String toolName, Environment env) throws IOException {
        ...
        if ("viewfile".equals(toolName)) {
            Path outputDir = env.tempHome.resolve("jad-output");
    ...
    private static String assertCallToolSuccess(String toolName, McpSchema.CallToolResult result) throws Exception {
        ...
        if (node != null && node.isObject()) {
            ...
            if (error != null && error.isBoolean() && error.booleanValue()) {
                Assertions.fail("tool  error=true: tool=" + toolName + ", body=" + text);
            }
            JsonNode status = node.get("status");
            if (status != null && status.isTextual() && "error".equalsIgnoreCase(status.asText())) {
                Assertions.fail("tool  status=error: tool=" + toolName + ", body=" + text);
            }
        }
    ...
    private static final class Environment implements AutoCloseable {
        ...
        private Environment(Path jadHome, Path tempHome, int httpPort, Process targetJvm,
        ...
        static Environment start(String clientName, String tempDirPrefix) throws Exception {
            ...
            try {
                ...
                return new Environment(jadHome, tempHome, httpPort, targetJvm, client, toolNames, targetClassFile);
            } catch (Exception e) {
                if (client != null) {
                    try {
    ...
    private static void runAttach(Path jadHome, Path tempHome, Path attachLog, long targetPid,
        ...
        ProcessBuilder pb = new ProcessBuilder(
                "bash",
                jadHome.resolve("jad.sh").toString(),
                "--attach-only",
                "--jad-home", jadHome.toString(),
                "--target-ip", "127.0.0.1",
    ...
    private static Path resolveTargetJvmAppClassFile() {
        String basedir = System.getProperty("basedir");
        assertThat(basedir).as("Maven surefire/failsafe should set system property 'basedir'").isNotBlank();
        return Paths.get(basedir, "target", "test-classes", "com", "akshita_sahu", "jad", "mcp", "it", "TargetJvmApp.class").normalize();
    }
