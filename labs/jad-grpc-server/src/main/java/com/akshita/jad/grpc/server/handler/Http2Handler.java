...
import jad.grpc.common.JADGrpc;
...
public class Http2Handler extends SimpleChannelInboundHandler<Http2Frame> {
    ...
    private void processError(ChannelHandlerContext ctx, Throwable e, Http2FrameStream stream) {
        ...
        JADGrpc.ErrorRes.Builder builder = JADGrpc.ErrorRes.newBuilder();
        JADGrpc.ErrorRes errorRes = builder.setErrorMsg(Optional.ofNullable(e.getMessage()).orElse("")).build();
        response.setClazz(JADGrpc.ErrorRes.class);
