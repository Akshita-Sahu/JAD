...
public class DownloadUtils {
    private static final String JAD_VERSIONS_URL = "https://github.com/Akshita-Sahu/JAD/api/versions";
    private static final String JAD_LATEST_VERSIONS_URL = "https://github.com/Akshita-Sahu/JAD/api/latest_version";

    private static final String JAD_DOWNLOAD_URL = "https://github.com/Akshita-Sahu/JAD/download/${VERSION}?mirror=${REPO}";
    ...
    public static String readLatestReleaseVersion() {
        ...
        try {
            URLConnection connection = openURLConnection(JAD_LATEST_VERSIONS_URL);
            inputStream = connection.getInputStream();
            return IOUtils.toString(inputStream).trim();
        } catch (Throwable t) {
            AnsiLog.error("Can not read jad version from: " + JAD_LATEST_VERSIONS_URL);
            AnsiLog.debug(t);
        } finally {
    ...
    public static List<String> readRemoteVersions() {
        InputStream inputStream = null;
        try {
            URLConnection connection = openURLConnection(JAD_VERSIONS_URL);
            inputStream = connection.getInputStream();
            ...
            AnsiLog.error("Can not read jad versions from: " + JAD_VERSIONS_URL);
            AnsiLog.debug(t);
        } finally {
            IOUtils.close(inputStream);
        }
    ...
    public static void downJADPackaging(String repoMirror, boolean http, String jadVersion, String savePath)
            throws IOException {
        String repoUrl = getRepoUrl(JAD_DOWNLOAD_URL, http);

        File unzipDir = new File(savePath, jadVersion + File.separator + "jad");

        File tempFile = File.createTempFile("jad", "jad");

        AnsiLog.debug("JAD download temp file: " + tempFile.getAbsolutePath());

        String remoteDownloadUrl = repoUrl.replace("${REPO}", repoMirror).replace("${VERSION}", jadVersion);
        AnsiLog.info("Start download jad from remote server: " + remoteDownloadUrl);
        saveUrl(tempFile.getAbsolutePath(), remoteDownloadUrl, true);
        AnsiLog.info("Download jad success.");
