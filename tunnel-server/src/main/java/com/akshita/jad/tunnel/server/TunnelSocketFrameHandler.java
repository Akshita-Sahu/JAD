...
import com.akshita.jad.tunnel.common.MethodConstants;
import com.akshita.jad.tunnel.common.SimpleHttpResponse;
import com.akshita.jad.tunnel.common.URIConstans;
import com.akshita.jad.tunnel.server.utils.HttpUtils;
...
public class TunnelSocketFrameHandler extends SimpleChannelInboundHandler<WebSocketFrame> {
    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
        if (evt instanceof HandshakeComplete) {
            ...
            if (MethodConstants.CONNECT_JAD.equals(method)) { // form browser
                connectJAD(ctx, parameters);
            } else if (MethodConstants.AGENT_REGISTER.equals(method)) { // form jad agent, register
                agentRegister(ctx, handshake, uri);
            }
            if (MethodConstants.OPEN_TUNNEL.equals(method)) { // from jad agent open tunnel
    ...
    protected void channelRead0(ChannelHandlerContext ctx, WebSocketFrame frame) throws Exception {
        ...
        if (frame instanceof TextWebSocketFrame) {
            ...
            /**
             * <pre>
             * 1. http proxy tunnel cleint， tunnel client，SimpleHttpResponse
             * 2.  URIConstans.PROXY_REQUEST_ID  Promise，SimpleHttpResponse
             * </pre>
             */
    ...
    private void connectJAD(ChannelHandlerContext tunnelSocketCtx, MultiValueMap<String, String> parameters)
            throws URISyntaxException {

        List<String> agentId = parameters.getOrDefault("id", Collections.emptyList());

        if (agentId.isEmpty()) {
            logger.error("jad agent id can not be null, parameters: {}", parameters);
            throw new IllegalArgumentException("jad agent id can not be null");
        }

        logger.info("try to connect to jad agent, id: " + agentId.get(0));
        ...
        if (findAgent.isPresent()) {
            ...
            promise.addListener(new FutureListener<Channel>() {
                ...
                public void operationComplete(final Future<Channel> future) throws Exception {
                    ...
                    if (future.isSuccess()) {
                        ...
                        // outboundChannel is form jad agent
            ...
            logger.info("browser connect waitting for jad agent open tunnel");
            boolean watiResult = promise.awaitUninterruptibly(20, TimeUnit.SECONDS);
            if (watiResult) {
                logger.info(
                        "browser connect wait for jad agent open tunnel success, agentId: {}, clientConnectionId: {}",
                        agentId, clientConnectionId);
            } else {
                logger.error(
                        "browser connect wait for jad agent open tunnel timeout, agentId: {}, clientConnectionId: {}",
                        agentId, clientConnectionId);
                tunnelSocketCtx.close();
            }
        } else {
            tunnelSocketCtx.channel().writeAndFlush(new CloseWebSocketFrame(2000, "Can not find jad agent by id: "+ agentId));
            logger.error("Can not find jad agent by id: {}", agentId);
            throw new IllegalArgumentException("Can not find jad agent by id: " + agentId);
        }
    }
    ...
    private void agentRegister(ChannelHandlerContext ctx, HandshakeComplete handshake, String requestUri) throws URISyntaxException {
        ...
        String jadVersion = null;
        List<String> jadVersionList = parameters.get(URIConstans.JAD_VERSION);
        if (jadVersionList != null && !jadVersionList.isEmpty()) {
            jadVersion = jadVersionList.get(0);
        }
