...
[`keymap` online tutorial](https://github.com/Akshita-Sahu/JAD/doc/jad-tutorials.html?language=en&id=command-keymap)
...
Specify customization in `$USER_HOME/.jad/conf/inputrc` file in the current user home directory.
...
then replace `"\C-h": backward-delete-char` with `"\C-h": backward-char`, then reconnect to JAD console to take effect.
