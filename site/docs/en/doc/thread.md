...
[`thread` online tutorial](https://github.com/Akshita-Sahu/JAD/doc/jad-tutorials.html?language=en&id=command-thread)
...
> Note: this operation consumes CPU time too (`getThreadCpuTime` is time-consuming), therefore it is possible to observe JAD's thread appears in the list. To avoid this, try to increase sample interval, for example: 5000 ms.<br/>
...
In some occasions, we experience the whole application is stuck because there's one particular thread hold one lock that other threads are relying on. To diagnose such an issue, JAD provides `thread -b` to find the problematic thread in one single command.
...
> Note: By now JAD only supports to locate the thread blocked by `synchronzied`, while `java.util.concurrent.Lock` is not supported yet.
