...
[`trace` online tutorial](https://github.com/Akshita-Sahu/JAD/doc/jad-tutorials.html?language=en&id=command-trace)
...
- Pls. also refer to [https://github.com/akshita-sahu/jad/issues/71](https://github.com/akshita-sahu/jad/issues/71) for more advanced usage
...
Many times what we are interested is the exact trace result when the method call takes time over one particular period. It is possible to achieve this in JAD, for example: `trace *StringUtils isBlank '#cost>100'` means trace result will only be output when the executing time exceeds 100ms.
...
- `trace` is handy to help discovering and locating the performance flaws in your system, but pls. note JAD can only trace the first level method call each time.

- After version 3.3.0, you can use the Dynamic Trace feature to add new matching classes/methods, see the following example.

- Currently `trace java.lang.Thread getName` is not supported, please refer to issue: [#1610](https://github.com/akshita-sahu/jad/issues/1610), considering that it is not very necessary and it is difficult to repair , So it won’t be fixed for now
...
- Here JAD provides the similar functionality JProfile and other commercial software provide. Compared to these professional softwares, JAD doesn't deduce the time cost `trace` itself takes, therefore it is not as accurate as these softwares offer. More classes and methods on the calling path, more inaccurate `trace` output is, but it is still helpful for diagnostics where the bottleneck is.
...
- The total time cost may not equal to the sum of the time costs each sub method call takes, this is because JAD instrumented code takes time too.
...
Now to drill down into the sub method `primeFactors`, you can open a new terminal 2 and use the `telnet localhost 3658` connects to the jad, then trace `primeFactors` with the specify `listenerId`.
