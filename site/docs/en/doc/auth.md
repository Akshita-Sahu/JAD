...
- The user can be specified by the `--username` option, the default value is `jad`.
- You can also configure username/password in `jad.properties`. The priority of the command line is higher than that of the configuration file.
- If only `username` is configured and no `password` is configured, a random password will be generated and printed in `~/logs/jad/jad.log`
...
By default, there are configurations in the `jad.properties` file:
...
After connecting to jad, directly executing the command will prompt for authentication:
...
JAD uses the HTTP standard Basic Authorization.
