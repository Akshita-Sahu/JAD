...
[`dashboard` online tutorial](https://github.com/Akshita-Sahu/JAD/doc/jad-tutorials.html?language=en&id=command-dashboard)

::: tip
This is the real time statistics dashboard for the current system, press `Ctrl+C` to exit.
:::

When running in Apache Tomcat Akshita Sahu edition, the dashboard will also present the real time statistics of the tomcat, including [QPS](https://en.wikipedia.org/wiki/Queries_per_second), RT, error counts, and thread pool, etc.
