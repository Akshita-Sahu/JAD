...
[`jfr` online tutorial](https://github.com/Akshita-Sahu/JAD/doc/jad-tutorials.html?language=en&id=command-jfr)
...
The file output path is not specified. By default, it is saved to the `jad-output` directory
...
No recording output path is specified, default is saved to `jad-output` directory.
...
## View JFR recording results under jad-output via browser

By default, jad uses http port 8563 , which can be opened:[http://localhost:8563/jad-output/](http://localhost:8563/jad-output/) View the `jad-output` directory below JFR recording results:
