...
[`profiler` online tutorial](https://github.com/Akshita-Sahu/JAD/doc/jad-tutorials.html?language=en&id=command-profiler)
...
When `--format md` (or `md=N`) is specified, JAD generates a structured Markdown report based on async-profiler `collapsed` output, which is easy to copy/paste to an LLM or grep in terminal. The report includes:
...
If `--file` ends with `.md` and `--format` is not specified, JAD will infer Markdown output automatically:
...
## View profiler results under jad-output via browser

By default, jad uses port 3658, which can be opened: [http://localhost:3658/jad-output/](http://localhost:3658/jad-output/) View the `jad-output` directory below Profiler results:
