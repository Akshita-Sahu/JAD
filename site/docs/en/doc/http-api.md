...
[`Http API` online tutorial](https://github.com/Akshita-Sahu/JAD/doc/jad-tutorials.html?language=en&id=case-http-api)
...
- `sessionId` : JAD session ID, one-time command does not need to
  set the session ID.
- `consumerId` : JAD consumer ID, used for multi-person sharing
  sessions.
...
For example, get the JAD version number:
...
- `state`: Request processing status, refer to the description of
  "Response Status".
- `sessionId `: JAD session ID, one-time command to automatically
...
- `type` : The command result type, except for the special ones such as
  `status`, the others remain the same as the JAD command name.
...
Users create and manage JAD sessions, which are suitable for complex
