...
[`options` online tutorial](https://github.com/Akshita-Sahu/JAD/doc/jad-tutorials.html?language=en&id=command-options)
...
| dump                   | false         | whether to dump enhanced class to the external files. If it's on, enhanced class will be dumped into `/${application dir}/jad-class-dump/`, the specific output path will be output in the console |
...
| support-default-method | true          | whether to enable matching default method in interface. The default value is `true`. Refer to [#1105](https://github.com/akshita-sahu/jad/issues/1105)                                                  |
| save-result            | false         | whether to save execution result. All execution results will be saved to `~/logs/jad-cache/result.log` when it's turned on                                                                         |
...
In order to prevent misuse like the above, JAD enables `strict` mode by default, in `ognl` expressions, it is forbidden to update the property of the object or call the `setter` method.
...
- For more information, please refer to: https://github.com/akshita-sahu/jad/issues/2128
