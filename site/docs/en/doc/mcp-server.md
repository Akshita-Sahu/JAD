...
JAD MCP Server is an experimental module of JAD that implements a server based on the [MCP (Model Context Protocol)](https://modelcontextprotocol.io/). This module provides a unified JSON-RPC 2.0 interface through HTTP/Netty, enabling AI assistants to execute JAD diagnostic commands via tool calls.

MCP (Model Context Protocol) is a standardized protocol proposed by Anthropic for connecting AI assistants with various tools and data sources. Through JAD MCP Server, AI assistants can naturally execute Java application diagnostic tasks, significantly improving development and operations efficiency.
...
JAD MCP Server integrates 29 diagnostic tools, categorized by functionality:
...
| **options**  | View or modify JAD global options                                                                                                            |

## Quick Start

### 1. Configure MCP Service

Enable MCP service in `jad.properties` configuration file:
...
Start JAD or Java application with JAD normally. By default, MCP service will be exposed on HTTP port 8563.
...
| -------------------- | --------------------------------------------------------------------------- | ------------------------------------ |
| `jad.mcpEndpoint` | MCP service access path                                                     | None (requires manual configuration) |
| `jad.mcpProtocol` | Transport protocol mode: `STREAMABLE` (stateful) or `STATELESS` (stateless) | `STREAMABLE`                         |
| `jad.httpPort`    | HTTP service port                                                           | 8563                                 |
| `jad.password`    | Authentication password (when authentication enabled)                       | None                                 |

### Transport Protocol Mode

JAD MCP Server supports two transport protocol modes:
...
When `jad.password` is configured in the configuration file, MCP Server will automatically enable authentication. In this case, you need to add an authentication header in the AI client configuration, with the Bearer Token set to the configured password value.

Configuration file example (`jad.properties`):
...
::: warning
**Note**: The token in the Authorization header must exactly match the value configured in `jad.password`.
:::
...
- The `jad-output` directory under the current working directory (if it exists)
...
::: tip
JAD MCP Server is an experimental feature. Feedback and suggestions are welcome!
:::

- **Issue Report**: [GitHub Issues](https://github.com/akshita-sahu/jad/issues)
- **Feature Request**: [GitHub Discussions](https://github.com/akshita-sahu/jad/discussions)
- **Contribute**: [Contributing Guide](https://github.com/akshita-sahu/jad/blob/master/CONTRIBUTING.md)
