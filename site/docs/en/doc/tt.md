...
[`tt` online tutorial](https://github.com/Akshita-Sahu/JAD/doc/jad-tutorials.html?language=en&id=command-tt)
...
- After using tt related functions, you need to manually release the memory, otherwise OOM may occur for a long time. Exiting jad will not automatically clear tt's cache map.
...
- `-n 3`
  ...
  limit the number of the records (avoid overflow for too many records; with `-n` option, JAD can automatically stop recording once the records reach the specified limit)
...
- [Special usage](https://github.com/akshita-sahu/jad/issues/71)
...
Since JAD stores the context of the call, you can even _replay_ the method calling afterwards with extra option `-p` to replay the issue for advanced troubleshooting, option `--replay-times`
...
Note that `com.akshita.jad.core.advisor.Advice#getLoader` is used here, and that it is better to use the exact `classloader` [ognl](ognl.md).

Advanced usage [get spring context to call the bean method](https://github.com/akshita-sahu/jad/issues/482)

F.Y.I

1. **Loss** of the `ThreadLocal`

   JAD save params into an array, then invoke the method with the params again. The method execute in another thread, so the `ThreadLocal` **lost**.

2. params may be modified

   JAD save params into an array, they are object references. The Objects may be modified by other code.
