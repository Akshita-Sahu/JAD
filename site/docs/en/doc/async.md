...
[`Async Jobs` online tutorial](https://github.com/Akshita-Sahu/JAD/doc/jad-tutorials?language=en&id=case-async-jobs)

Asynchronous jobs in jad. The idea is borrowed from [linux jobs](http://man7.org/linux/man-pages/man1/jobs.1p.html).
...
- ‘ctrl + d’: According to linux semantics this should lead to exit the terminal, right now JAD has not implemented this yet, therefore simply ignore this keystroke.
...
If no redirect file is specified, the result will be output to the `~/logs/jad-cache/` directory, for example:
...
At this time, the command will be executed asynchronously in the background, and the result will be asynchronously saved in the file (`~/logs/jad-cache/${PID}/${JobId}`);
...
- The result of this command will be output asynchronously to the file; at this time, regardless of whether `save-result` is true or not, the result will not be written asynchronously to `~/logs/jad-cache/result.log`.
...
- If you do not want to stop the JAD service and continue to perform background tasks, you can exit the JAD console by executing `quit` command (`stop` command will stop the JAD service)
