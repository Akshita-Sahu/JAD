...
const getStarForkData = async () => {
  const stars = await fetch("https://github.com/Akshita-Sahu/JAD/api/starCount").then(
    (res) => res.json(),
  );

  const forks = await fetch("https://github.com/Akshita-Sahu/JAD/api/forkCount").then(
    (res) => res.json(),
  );
